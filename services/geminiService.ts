...
export const reimagineImage = async (
    ...
    try {
        ...
        if (!description) {
            throw new Error("مدل هوش مصنوعی نتوانست تصویر را توصیف کند.");
        }
        
        console.log("Generated description from Gemini:", description);

        // Step 2: Generate a new image using a structured prompt
        const finalPrompt = `A hyper-realistic, 8k, cinematic, dramatic lighting, sharp focus photo of: ${description}. The subject is in a dynamic, powerful pose.`;
        ...
        if (error instanceof Error) {
            throw new Error(`متاسفانه در بازآفرینی تصویر مشکلی پیش آمد: ${error.message}`);
        }
